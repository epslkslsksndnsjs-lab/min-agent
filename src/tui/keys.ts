...
/** Set the global Kitty keyboard protocol state. Called by the terminal after detecting support. */
...
export function isKeyRelease(data: string): boolean {
  ...
  // patterns like ":3F" (e.g. MAC addresses like "90:62:3F:A5"). Paste content
  // is always re-wrapped with bracketed paste markers before reaching here.
...
function matchesKittySequence(data: string, expectedCodepoint: number, expectedModifier: number): boolean {
  ...
  // Only fall back when the codepoint is NOT already a recognized Latin letter
  // or symbol — a recognized codepoint is authoritative regardless of physical
  // key position, which prevents remapped layouts from causing false matches.
 ...
 * Used by terminals when the Kitty protocol is not enabled.
 */
...
/**
 * Raw 0x08 (BS) is ambiguous in legacy terminals: Windows Terminal uses it
 * for Ctrl+Backspace, some legacy terminals and tmux send it for plain
 * Backspace. Fall back to a Windows Terminal heuristic only for raw BS bytes.
 */
...
/**
 * Get the control character for a key using the universal formula: code & 0x1f.
 */
  ...
  switch (key) {
    ...
    case 'return':
      if (modifier === MODIFIERS.shift) {
        ...
        if (_kittyProtocolActive) {
          // Custom terminal mappings: Kitty's "map shift+enter send_text all \e\r"
          // and Ghostty's "keybind = shift+enter=text:\n"
...
function formatParsedKey(codepoint: number, modifier: number, baseLayoutKey?: number): string | undefined {
  ...
  // regardless of physical key position (prevents remapped-layout false names).
...
export function parseKey(data: string): string | undefined {
  ...
  // Mode-aware legacy sequences. When the Kitty protocol is active, ambiguous
  // sequences are interpreted as custom terminal mappings:
 ...
 *
 * When the Kitty keyboard protocol flag 1 (disambiguate) is active, terminals
 * send CSI-u sequences for all keys, including plain printable characters.
 * Only plain or Shift-modified keys are accepted; Ctrl/Alt-modified sequences
 * are handled by keybinding matching instead. Prefers the shifted keycode when
 * Shift is held and a shifted key is reported.
 */
